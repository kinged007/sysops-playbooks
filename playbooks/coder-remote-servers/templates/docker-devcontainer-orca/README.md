...
## ORCA experiment (`docker-devcontainer-orca`)

Experimental copy of `docker-devcontainer` for hosting an Orca remote server
inside a workspace. Only addition: optional published port for the in-workspace
Orca serve (container `6768` → host `6769/tcp`, bound to `var.orca_publish_ip`).

Push example (remote daemon on the Orca host, tailnet-only bind):

```sh
coder templates push 24gb-169-58-docker-orca ./templates/docker-devcontainer-orca \
  --var docker_host=tcp://<HOST-TAILSCALE-IP>:2376 \
  --var docker_ca="$(cat <certs-dir>/ca.pem)" \
  --var docker_cert="$(cat <certs-dir>/cert.pem)" \
  --var docker_key="$(cat <certs-dir>/key.pem)" \
  --var orca_publish_ip=<HOST-TAILSCALE-IP>
```

Placeholders only — real IPs live in the execution folder / template variables,
never in this repo. Serve inside the workspace with
`orca serve --pairing-address <HOST-TAILSCALE-IP> --port 6769`; clients pair
over the tailnet (grant `tcp:6769` required). No systemd in these containers —
start serve from the workspace (startup script in Phase 2).
